 ...
 * v1.0.0(2016.9.8):
 初始生成
 * ---------------------------------------------------------------------------*/
KISSY.add('page/history-order', function (S, Scroll, Core) {
    PW.namespace('page.Order');
    PW.page.Order = function (param) {
        new Scroll();
        new Core();
    }
}, {
...
/*滚动条相关操作*/
KISSY.add('order/scroll', function (S) {
    var
        DOM = S.DOM,
        on = S.Event.on,
        Juicer = PW.mod.Juicer.client,
        fire = S.Event.fire,
        OrderIO = PW.io.HistoryOrder,
        el = {
            //渲染刷分页模板
...
KISSY.add('order/core', function (S) {
    ...
    S.augment(Core, {
        ...
        _del: function (e) {
            var
                that = this,
                confirmDelOrder = confirm("确定删除该订单？"),
                liEl = DOM.parents(e, 'li');

            if(confirmDelOrder == true){
                OrderIO.delOrder({
                    id: DOM.attr(liEl, DATA_ORDER_ID)
                }, function (rs, errMsg) {
                    if (rs) {
                        $(liEl).slideUp(0.2, function () {
                            DOM.remove(liEl);
                        })
                    } else {
                        alert(errMsg);
